...
describe('HMAC Utilities', () => {
  ...
  describe('verifyHmacSignature', () => {
    ...
    it('rejects an invalid signature', () => {
      ...
      const isValid = verifyHmacSignature(invalidSignature, testPayload, testSecret);
    ...
    it('rejects a signature with wrong secret', () => {
      const signature = generateHmacSignature(testPayload, testSecret);
      const isValid = verifyHmacSignature(signature, testPayload, 'wrong-secret');
    ...
    it('rejects a signature with modified payload', () => {
      ...
      const isValid = verifyHmacSignature(signature, modifiedPayload, testSecret);
  ...
  describe('verifyHmacSignatureWithRotation', () => {
    ...
    it('verifies signature with active key', () => {
      ...
      const result = verifyHmacSignatureWithRotation(signature, testPayload, activeKey, nextKey);
    ...
    it('rejects invalid signature', () => {
      const invalidSignature = 'a'.repeat(64);
      const result = verifyHmacSignatureWithRotation(invalidSignature, testPayload, activeKey, nextKey);
    ...
    it('works without next key', () => {
      const signature = generateHmacSignature(testPayload, activeKey);
      const result = verifyHmacSignatureWithRotation(signature, testPayload, activeKey);
