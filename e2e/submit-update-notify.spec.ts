...
test.describe('Submit → Update → Notify Flow', () => {
  test('should complete full flow: submit request, view in dashboard, update status', async ({ page }) => {
    ...
    await expect(page.getByText(/request submitted successfully/i)).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByRole('button', { name: /copy tracking code/i })).toBeVisible();
    
    // Step 4: Navigate to dashboard
    await page.goto('/dashboard');
    
    // Wait for dashboard to load
    await expect(page.getByRole('heading', { name: /responder dashboard/i })).toBeVisible();
